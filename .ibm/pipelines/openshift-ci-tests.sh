...
initiate_rds_deployment() {
  ...
  sed -i "s|POSTGRES_USER:.*|POSTGRES_USER: $(echo -n $RDS_USER | base64 -w 0)|g" "${DIR}/resources/postgres-db/postgres-cred.yaml"
  ...
  oc apply -f "$DIR/resources/postgres-db/dynamic-plugins-root-PVC.yaml
